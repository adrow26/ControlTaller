...
def main(page: ft.Page):
    ...
    page.window.width = 450  # Fix: antes era window_width
    page.scroll = ft.ScrollMode.AUTO
    
    # FilePicker obligatorio para descargar PDF en web
    ...
    # Tabla trabajos - expand=True para que no se bloquee después de 4 órdenes
    tabla = ft.DataTable(
        columns=[
    ...
    lista_tabla = ft.ListView(expand=True, spacing=5)  # expand=True clave
        ...
        for t in ordenes:
            tabla.rows.append(
                ft.DataRow(cells=[
                    ...
                    # Fix: convertir a float para evitar ValueError
                    ft.DataCell(ft.Text(f"Bs {float(t.get('costo_repuesto', 0)):.2f}")),
                    ft.DataCell(ft.Text(f"Bs {float(t['total']):.2f}")),
                ])
            )
        page.update()
    ...
    def agregar_orden(e):
        ...
        orden = {
            ...
            # Fix: forzar float al guardar
            "costo_repuesto": float(txt_costo.value or 0),
    ...
    def generar_pdf(path):
        ...
        table = Table(data, colWidths=[50, 100, 120, 120, 80])
    ...
    def save_file_result(e: ft.FilePickerResultEvent):
        if e.path:
            generar_pdf(e.path)
            page.snack_bar = ft.SnackBar(ft.Text(f"PDF guardado en {e.path}"))
            page.snack_bar.open = True
            page.update()
